...
- **A correction is a commit, not a release.** Fix the source HTML in `Protocols/` or `Procedures/`, run `npm run build`, commit, push. `.github/workflows/build-and-check.yml` also does this automatically on every push that touches source content — it re-extracts, commits the rebuilt `dist/`, and purges the jsDelivr cache so the fix is usually live within seconds, no App Store review and no PWA redeploy involved. Observed live once: purging immediately after a push can race GitHub's own propagation of the new commit to `raw.githubusercontent.com`, which can silently re-cache the *old* version even though the purge call itself reported success — the workflow purges twice, 15s apart, to catch that; if content still looks stale a few minutes after a push, purge manually: `curl https://purge.jsdelivr.net/gh/awpeace1906-collab/crisis-content@main/dist/<file>.json`.
